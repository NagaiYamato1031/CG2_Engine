...
void Sprite::StaticInitialize(ID3D12Device* device, int windowWidth, int windowHeight)
{
	...
	// 頂点シェーダをコンパイルする
	vertexBlob = dxc_->CompileShader(L"./resources/shaders/Sprite.VS.hlsl", L"vs_6_0");
	// コンパイル出来たか確認する
	assert(vertexBlob != nullptr);
	// ピクセルシェーダをコンパイルする
	pixelBlob = dxc_->CompileShader(L"./resources/shaders/Sprite.PS.hlsl", L"ps_6_0");
	// コンパイル出来たか確認する
